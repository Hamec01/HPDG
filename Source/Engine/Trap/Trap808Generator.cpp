...
BassPhraseRole classifyBassRole(int stepInBar, TrapPhraseRole phraseRole)
{
    if (stepInBar == 0 || stepInBar == 8)
        return BassPhraseRole::Anchor;
    if (phraseRole == TrapPhraseRole::Ending || stepInBar >= 14)
        return BassPhraseRole::Ending;
    if (stepInBar <= 2 || stepInBar == 7)
...
std::vector<int> buildDegreePool(const TrapStyleSpec& spec, BassPhraseRole role, int scaleMode)
{
    std::vector<int> pool;
    pool.push_back(0);

    const bool isMinorLike = scaleMode != 1;
    const int third = isMinorLike ? 2 : 2;
    const int fifth = 4;
    const int seventh = isMinorLike ? 6 : 6;

    switch (spec.pitchMotion)
    {
        case TrapPitchMotion::VeryStatic:
            if (role == BassPhraseRole::Ending)
                pool.push_back(fifth);
            break;

        case TrapPitchMotion::StaticWithAccentMoves:
            pool.push_back(fifth);
            if (role == BassPhraseRole::Ending || role == BassPhraseRole::Response)
                pool.push_back(third);
            break;

        case TrapPitchMotion::ControlledMovement:
            pool.push_back(fifth);
            pool.push_back(third);
            if (role == BassPhraseRole::Ending)
        ...
        case TrapPitchMotion::EnergeticControlledMovement:
            pool.push_back(fifth);
            pool.push_back(third);
            pool.push_back(seventh);
            break;
                   ...
                   std::mt19937& rng)
{
    ...
    if (previousPitch > 0 && (role == BassPhraseRole::Anchor || role == BassPhraseRole::Support) && chance(rng) < 0.66f)
    {
    ...
    int octaveShift = 0;
    if (role == BassPhraseRole::Ending && chance(rng) < 0.42f)
        octaveShift = -12;
    else if (role == BassPhraseRole::Pickup && chance(rng) < 0.16f)
        octaveShift = 12;
                                      ...
                                      std::mt19937& rng) const
{
    ...
    const float followKick = std::clamp(spec.followKickProbability * activity, 0.1f, 0.98f);
    const float sustain = std::clamp(spec.sustainProbability, 0.08f, 0.94f);
    const float counterGap = std::clamp(spec.counterGapProbability * (spec.preferSparseSpace ? 0.8f : 1.0f), 0.0f, 0.72f);
    ...
    for (const auto& k : kickTrack.notes)
    {
        ...
        if (spec.rhythmMode == Trap808Mode::LayerMostKicks)
            reinforce += 0.12f;
        else if (spec.rhythmMode == Trap808Mode::SparseCounter)
            reinforce -= 0.12f;
        if (phraseEdge)
        ...
        {
            if (spec.rhythmMode == Trap808Mode::PhraseAnchored)
                anchorLength = phraseEdge ? 4 : 3;
            else if (spec.rhythmMode == Trap808Mode::LayerMostKicks)
                anchorLength = 2;
            else
                anchorLength = phraseEdge ? 3 : 2;
        }

        if (chance(rng) < std::clamp(reinforce, 0.08f, 0.99f))
            subTrack.notes.push_back({ 36, k.step, anchorLength, vel(rng), 0, false });

        if (stepInBar < 15 && chance(rng) < std::clamp(counterGap, 0.0f, 0.92f))
            subTrack.notes.push_back({ 36, k.step + 1, 1, std::max(style.sub808VelocityMin, vel(rng) - 10), 0, false });

        if (spec.rhythmMode != Trap808Mode::SparseCounter && stepInBar > 1 && chance(rng) < 0.1f * activity)
            subTrack.notes.push_back({ 36, k.step - 1, 1, std::max(style.sub808VelocityMin, vel(rng) - 12), 0, false });

        if (phraseRole == TrapPhraseRole::Ending && chance(rng) < std::clamp(spec.phraseEdgeAnchorProbability, 0.12f, 0.9f))
        {
                                   ...
                                   std::mt19937& rng) const
{
    ...
    for (size_t i = 0; i + 1 < subTrack.notes.size(); ++i)
    {
        ...
        if (phraseRole == TrapPhraseRole::Ending || stepInBar >= 13)
            gate += 0.1f;
